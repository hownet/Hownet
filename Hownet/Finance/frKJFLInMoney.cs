...
{
    public partial class frKJFLInMoney :  DevExpress.XtraEditors.XtraForm
    {
        public frKJFLInMoney()
        {
            InitializeComponent();
        }
        ...
        public frKJFLInMoney(int MainID)
            : this()
        {
            _MainID = MainID;
        }
        ...
        void InData()
        {
            dtMain = BasicClass.GetDataSet.GetDS(bllMain, "GetIDList",new object[]{1}).Tables[0];
        ...
        private void _barGZ_ItemClick(object sender, DevExpress.XtraBars.ItemClickEventArgs e)
        {
            ...
            for (int i = 0; i < dtInfo.Rows.Count; i++)
            {
                try
                {
                    ...
                    if (dtKM.Rows.Count > 0)
                    {
                        IsFuZai = dtKM.Rows[0]["Num"].ToString().Substring(0, 1) == "2";
                        if (IsFuZai)
                            dtKM.Rows[0]["Money"] = Convert.ToDecimal(dtKM.Rows[0]["Money"]) + Convert.ToDecimal(dtInfo.Rows[i]["金额"]);
                        else
            ...
            dr["TableID"] = _MainID;
            dr["TypeID"] = (int)BasicClass.Enums.MoneyTableType.KJFLInMoney ;
        ...
        private void _barTuiZhang_ItemClick(object sender, DevExpress.XtraBars.ItemClickEventArgs e)
        {
            ...
            {
                MessageBox.Show("此单所不能退帐，请到原始单据中弃审！");
                return;
            }
